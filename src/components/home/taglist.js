...
const TagList = () => {
  ...
  const getTags = async() => {
    ...
    if(result) {
      result.sort((a,b)=>a.count<b.count? 1 : a.count>b.count? -1 : 0)
      setTags(result)
    }
  }

  const sortTags = (how) => {
    if(how==='name') tags.sort((a,b)=>a.tagName>b.tagName? 1 : a.tagName<b.tagName? -1 : 0)
    else if(how==='count') tags.sort((a,b)=>a.count<b.count? 1 : a.count>b.count? -1 : 0)
  }
      ...
      <TagContainer>
        {tags.map((item,index)=> 
          <TagItem key={`tag ${index}`}>
            <TagItemName>{item.tagName}</TagItemName>
            <TagItemCount>{item.count}</TagItemCount>
         </TagItem>
        )}
      </TagContainer>
    </Container>
